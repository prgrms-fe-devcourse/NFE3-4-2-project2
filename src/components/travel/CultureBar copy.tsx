...
const CultureBar: React.FC<CultureBarProps> = ({ selectedCulture, onCultureSelect }) => {
   const cultureData = [
      { culture: "미술관·박물관", imageSrc: "/images/culture/museum.png", description: "예술과 역사를 한눈에" },
      { culture: "유적지", imageSrc: "/images/culture/historic.png", description: "과거를 따라 유적지 탐방" },
      { culture: "종교", imageSrc: "/images/culture/religion.png", description: "신성한 여정, 종교 성지" },
      { culture: "기타", imageSrc: "/images/culture/etc.png", description: "다양한 매력, 특별한 장소" },
   ];
         ...
         {cultureData.map((culture) => (
            <div
               ...
               className={`relative flex flex-col items-center cursor-pointer transition-all p-4 rounded-md ${
                  selectedCulture === culture.culture ? "scale-105" : "hover:scale-105 hover:shadow-xl"
               }`}
